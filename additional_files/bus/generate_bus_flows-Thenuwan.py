...
#          "20_hotel_road", "21_odeon_cinema", "22_dehiwala_junc", "23_waydya_rd", "24_william_junc", "25_roxi_cinema",
...
# # sd_mean = 1
# # sd_std = 0.1
# ld_stop_time = np.random.normal(ld_mean, ld_std, 100).tolist()
# sd_stop_time = np.random.normal(sd_mean, sd_std, 100).tolist()
# # Gamma distribution
# ld_shape = np.square(ld_mean / ld_std)
# ld_scale = ld_mean / np.square(ld_std)
#
# sd_shape = np.square(sd_mean / sd_std)
# sd_scale = np.square(sd_mean / sd_std)
#
# # ld_stop_time = np.random.gamma(ld_shape, ld_scale, 100).tolist()
# # sd_stop_time = np.random.gamma(sd_shape, sd_scale, 100).tolist()
#
# ld_stop_time_format = [0 if x < 0 else x for x in ld_stop_time]
# sd_stop_time_format = [0 if x < 0 else x for x in sd_stop_time]
#
# ld_stop_time_format = ['%.0f' % elem for elem in ld_stop_time_format]
# sd_stop_time_format = ['%.0f' % elem for elem in sd_stop_time_format]
#
# # now I can loop through and create bus stopping time as per my previous code created!
#
...
#         'maxSpeed="{}" color="red" '
#         'guiShape="bus"/>\n'.format(SIGMA, MAX_SPEED_SHORT_DISTANCE))
#
#     # long distance bus type
#     fh.write(
#         '\t<vType id="long_distance" vClass="bus" accel="2.6" decel="4.5" sigma="{}" length="12" minGap="3" '
#         'maxSpeed="{}" color="red" '
#         'guiShape="bus"/>\n'.format(SIGMA, MAX_SPEED_LONG_DISTANCE))
#
#     # writing xml lines
#     for index_o, row_o in all_bus_route_df.iterrows():
#         trip_id = row_o['trip_id']
#         bus_type = row_o['bus_type']
#         vehicle_depart = row_o['depart']
#         start_edge = row_o['start_edge']
#         end_edge = row_o['end_edge']
#         bus_stops = row_o['bus_stops']
#
#         fh.write('\t<trip id="{}" type="{}" depart="{}" from="{}" to="{}">\n'.format(
#             trip_id, bus_type, vehicle_depart, start_edge, end_edge))
#
#         for stop in bus_stops:
#             stop_duration = stopping_duration(bus_type, ld_stop_time_format, sd_stop_time_format)
#             fh.write('\t\t<stop busStop="{}" duration="{}"/>\n'.format(stop, stop_duration))
#
#         fh.write('\t</trip>\n')
#     fh.write('</additional>\n')
#     fh.close()
